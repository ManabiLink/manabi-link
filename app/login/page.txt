    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.access_token) {
          // sessionStorageやメモリに保存（localStorageは使用不可）
          console.log('Access token:', data.access_token);
        }
      } else {
        showMessage(data.error_description || 'ログインに失敗しました', 'error');
      }
    } catch (error) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showMessage(data.error_description || '登録に失敗しました', 'error');
      }
    } catch (error) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        showMessage(data.error_description || 'メール送信に失敗しました', 'error');
      }
    } catch (error) {
